...
def train(args):
    ...
    train_loader = data.DataLoader(SQuID(args, 'train', args.data_dir, is_training_set=True),
    ...
    while not logger.is_finished_training():
        ...
        for src, src_c, bio, tgt in train_loader:
            logger.start_iter()
            with torch.set_grad_enabled(True):
                logits = model.forward(src.to(args.device), src_c.to(args.device), bio.to(args.device))
